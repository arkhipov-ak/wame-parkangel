...
const AdminInfoUser = () => {
  ...
  const getUser = () => {
    axios
      .get(`https://api.parkangel.ru/api/users/${query.id}`, {
        params: {
          id: query.id,
        },
      })
  ...
  const onHandleBlockUser = () => {
    axios
      ...
      .then(() => {
        showSuccessSnackbar({ message: "Пользователь заблокирован" });
        navigate("/admin/info");
      })
      .catch(() => {
        showErrorSnackbar({ message: "Не удалось заблокировать пользователя" });
      });
  };
  ...
  return (
    <>
      <NavBar />
      <Container>
        <Button onClick={() => onHandleBlockUser()}>Заблокировать</Button>
        <>
          {adsArray.length ? (
            <>
