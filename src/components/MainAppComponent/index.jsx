...
const MainAppComponent = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      axios
        .get(`https://api.parkangel.ru/api/users/chatId/${chatId}`)
        .then((response) => {
          if (response.data.response) state.user = response.data.response;
          else {
            axios
              .post("https://api.parkangel.ru/api/users", {
                chatId: chatId + "",
              ...
              .catch(() =>
                showErrorSnackbar({ message: "Не удалось записать юзера" })
              );
          }
        })
  ...
  useEffect(() => {
    if (snap.user) {
      axios
        .get(`https://api.parkangel.ru/api/options/userId/${snap.user.id}`)
        .then((response) => (state.options = response.data.response))
        .catch(() =>
          showErrorSnackbar({ message: "Не удалось загрузить опции" })
        );
    }
  }, [snap.user?.id]);

  if (snap.user?.is_blocked) {
    return (
      <div data-theme={snap.user?.theme || "light"} style={{ width: "100vw" }}>
        <div className={styles.container}>
          <img src={Logotype} alt="Логотип" className={styles.img} />
          <h2 className={styles.text}>Ваш аккаунт заблокирован</h2>
        </div>
      </div>
    );
  }
      ...
      >
        ...
        <Routes>
          ...
          <Route
            ...
            element={<ResultSearchElementPage />}
          />
